...
# Check for exact recurrence keywords
    ...
    # Check for explicit weekday recurrence after every
    m = re.search(r'\bevery\s+((?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)(?:\s+(?:and\s+)?(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday))*)\b', text_lower)
    if m:
        weekday_part = m.group(1)
        found_days = []
        for day_name in WEEKDAYS.keys():
            if re.search(rf'\b{re.escape(day_name)}\b', weekday_part):
                found_days.append(day_name)
        if found_days:
            return f'weekly_on_{"_".join(found_days[:3])}', warnings

    # Dashed or comma-separated weekday list
    m = re.search(r'\bevery\s+([a-z,\s]+?)(?:\s+at|\s+on|\s+from|\s+for|$)', text_lower)
    if m:
